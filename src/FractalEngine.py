...
from InterfaceItems import Button, Slider, CheckBox
...
class Fractal2D(QOpenGLWidget):
    """ Абстрактный класс для 2д фрактала. """
    def __init__(self,
                 window: QMainWindow,
        ...
        :param opengl_layout: место для виджета с фракталом.
        :param buttons_layout: место для кнопок управления.
        :param object_name: название фрактала.
    ...
    def initializeGL(self):
        ...
        glVertexAttribPointer(in_shader_pos, 2, GL_FLOAT, GL_FALSE, vertices.itemsize * 2, ctypes.c_void_p(0))
    ...
    def animate(self):
        ...
        self.draw = True
        current = self.settings["ANIMATION_PARAM"]
        self.settings[current] = self.animation_params[current](self.settings[current],
...
class Fractal3D(Fractal2D):
    def save_position(self):
        with open("res/doc/positions.txt", "a") as file:
            text = "; ".join([f'{self.settings["PHI"]}', f'{self.settings["THETA"]}',
                              f'{self.settings["ZOOM"]}'])
            file.write(text + "\n")
    ...
    def input_position(self):
        ...
        if ok:
            ...
            for i in range(3):
                ...
                val = float(text[i])
                if i == 0:
                    self.settings["PHI"] = val
                elif i == 1:
                    self.settings["THETA"] = val
                else:
                    self.settings["ZOOM"] = val
